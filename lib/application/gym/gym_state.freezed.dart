// coverage:ignore-file
// GENERATED CODE - DO NOT MODIFY BY HAND
// ignore_for_file: type=lint
// ignore_for_file: unused_element, deprecated_member_use, deprecated_member_use_from_same_package, use_function_type_syntax_for_parameters, unnecessary_const, avoid_init_to_null, invalid_override_different_default_values_named, prefer_expression_function_bodies, annotate_overrides, invalid_annotation_target, unnecessary_question_mark

part of 'gym_state.dart';

// **************************************************************************
// FreezedGenerator
// **************************************************************************

T _$identity<T>(T value) => value;

final _privateConstructorUsedError = UnsupportedError(
    'It seems like you constructed your class using `MyClass._()`. This constructor is only meant to be used by freezed and you are not supposed to need it nor use it.\nPlease check the documentation here for more information: https://github.com/rrousselGit/freezed#custom-getters-and-methods');

/// @nodoc
mixin _$GymState {
  bool get isloading => throw _privateConstructorUsedError;
  GymResponse? get gym => throw _privateConstructorUsedError;
  List<String>? get activities => throw _privateConstructorUsedError;
  String? get selectedActivity => throw _privateConstructorUsedError;
  List<dynamic>? get photos => throw _privateConstructorUsedError;
  List<String> get originalDates => throw _privateConstructorUsedError;
  Map<String, dynamic> get scheddules => throw _privateConstructorUsedError;
  List<dynamic> get availableDates => throw _privateConstructorUsedError;
  List<dynamic> get listOfSchedules => throw _privateConstructorUsedError;
  String get defaultDate => throw _privateConstructorUsedError;
  String get selectedOriginalDate => throw _privateConstructorUsedError;
  List<String> get listOfDates => throw _privateConstructorUsedError;
  String get selectedDay => throw _privateConstructorUsedError;

  @JsonKey(ignore: true)
  $GymStateCopyWith<GymState> get copyWith =>
      throw _privateConstructorUsedError;
}

/// @nodoc
abstract class $GymStateCopyWith<$Res> {
  factory $GymStateCopyWith(GymState value, $Res Function(GymState) then) =
      _$GymStateCopyWithImpl<$Res, GymState>;
  @useResult
  $Res call(
      {bool isloading,
      GymResponse? gym,
      List<String>? activities,
      String? selectedActivity,
      List<dynamic>? photos,
      List<String> originalDates,
      Map<String, dynamic> scheddules,
      List<dynamic> availableDates,
      List<dynamic> listOfSchedules,
      String defaultDate,
      String selectedOriginalDate,
      List<String> listOfDates,
      String selectedDay});
}

/// @nodoc
class _$GymStateCopyWithImpl<$Res, $Val extends GymState>
    implements $GymStateCopyWith<$Res> {
  _$GymStateCopyWithImpl(this._value, this._then);

  // ignore: unused_field
  final $Val _value;
  // ignore: unused_field
  final $Res Function($Val) _then;

  @pragma('vm:prefer-inline')
  @override
  $Res call({
    Object? isloading = null,
    Object? gym = freezed,
    Object? activities = freezed,
    Object? selectedActivity = freezed,
    Object? photos = freezed,
    Object? originalDates = null,
    Object? scheddules = null,
    Object? availableDates = null,
    Object? listOfSchedules = null,
    Object? defaultDate = null,
    Object? selectedOriginalDate = null,
    Object? listOfDates = null,
    Object? selectedDay = null,
  }) {
    return _then(_value.copyWith(
      isloading: null == isloading
          ? _value.isloading
          : isloading // ignore: cast_nullable_to_non_nullable
              as bool,
      gym: freezed == gym
          ? _value.gym
          : gym // ignore: cast_nullable_to_non_nullable
              as GymResponse?,
      activities: freezed == activities
          ? _value.activities
          : activities // ignore: cast_nullable_to_non_nullable
              as List<String>?,
      selectedActivity: freezed == selectedActivity
          ? _value.selectedActivity
          : selectedActivity // ignore: cast_nullable_to_non_nullable
              as String?,
      photos: freezed == photos
          ? _value.photos
          : photos // ignore: cast_nullable_to_non_nullable
              as List<dynamic>?,
      originalDates: null == originalDates
          ? _value.originalDates
          : originalDates // ignore: cast_nullable_to_non_nullable
              as List<String>,
      scheddules: null == scheddules
          ? _value.scheddules
          : scheddules // ignore: cast_nullable_to_non_nullable
              as Map<String, dynamic>,
      availableDates: null == availableDates
          ? _value.availableDates
          : availableDates // ignore: cast_nullable_to_non_nullable
              as List<dynamic>,
      listOfSchedules: null == listOfSchedules
          ? _value.listOfSchedules
          : listOfSchedules // ignore: cast_nullable_to_non_nullable
              as List<dynamic>,
      defaultDate: null == defaultDate
          ? _value.defaultDate
          : defaultDate // ignore: cast_nullable_to_non_nullable
              as String,
      selectedOriginalDate: null == selectedOriginalDate
          ? _value.selectedOriginalDate
          : selectedOriginalDate // ignore: cast_nullable_to_non_nullable
              as String,
      listOfDates: null == listOfDates
          ? _value.listOfDates
          : listOfDates // ignore: cast_nullable_to_non_nullable
              as List<String>,
      selectedDay: null == selectedDay
          ? _value.selectedDay
          : selectedDay // ignore: cast_nullable_to_non_nullable
              as String,
    ) as $Val);
  }
}

/// @nodoc
abstract class _$$GymStateImplCopyWith<$Res>
    implements $GymStateCopyWith<$Res> {
  factory _$$GymStateImplCopyWith(
          _$GymStateImpl value, $Res Function(_$GymStateImpl) then) =
      __$$GymStateImplCopyWithImpl<$Res>;
  @override
  @useResult
  $Res call(
      {bool isloading,
      GymResponse? gym,
      List<String>? activities,
      String? selectedActivity,
      List<dynamic>? photos,
      List<String> originalDates,
      Map<String, dynamic> scheddules,
      List<dynamic> availableDates,
      List<dynamic> listOfSchedules,
      String defaultDate,
      String selectedOriginalDate,
      List<String> listOfDates,
      String selectedDay});
}

/// @nodoc
class __$$GymStateImplCopyWithImpl<$Res>
    extends _$GymStateCopyWithImpl<$Res, _$GymStateImpl>
    implements _$$GymStateImplCopyWith<$Res> {
  __$$GymStateImplCopyWithImpl(
      _$GymStateImpl _value, $Res Function(_$GymStateImpl) _then)
      : super(_value, _then);

  @pragma('vm:prefer-inline')
  @override
  $Res call({
    Object? isloading = null,
    Object? gym = freezed,
    Object? activities = freezed,
    Object? selectedActivity = freezed,
    Object? photos = freezed,
    Object? originalDates = null,
    Object? scheddules = null,
    Object? availableDates = null,
    Object? listOfSchedules = null,
    Object? defaultDate = null,
    Object? selectedOriginalDate = null,
    Object? listOfDates = null,
    Object? selectedDay = null,
  }) {
    return _then(_$GymStateImpl(
      isloading: null == isloading
          ? _value.isloading
          : isloading // ignore: cast_nullable_to_non_nullable
              as bool,
      gym: freezed == gym
          ? _value.gym
          : gym // ignore: cast_nullable_to_non_nullable
              as GymResponse?,
      activities: freezed == activities
          ? _value._activities
          : activities // ignore: cast_nullable_to_non_nullable
              as List<String>?,
      selectedActivity: freezed == selectedActivity
          ? _value.selectedActivity
          : selectedActivity // ignore: cast_nullable_to_non_nullable
              as String?,
      photos: freezed == photos
          ? _value._photos
          : photos // ignore: cast_nullable_to_non_nullable
              as List<dynamic>?,
      originalDates: null == originalDates
          ? _value._originalDates
          : originalDates // ignore: cast_nullable_to_non_nullable
              as List<String>,
      scheddules: null == scheddules
          ? _value._scheddules
          : scheddules // ignore: cast_nullable_to_non_nullable
              as Map<String, dynamic>,
      availableDates: null == availableDates
          ? _value._availableDates
          : availableDates // ignore: cast_nullable_to_non_nullable
              as List<dynamic>,
      listOfSchedules: null == listOfSchedules
          ? _value._listOfSchedules
          : listOfSchedules // ignore: cast_nullable_to_non_nullable
              as List<dynamic>,
      defaultDate: null == defaultDate
          ? _value.defaultDate
          : defaultDate // ignore: cast_nullable_to_non_nullable
              as String,
      selectedOriginalDate: null == selectedOriginalDate
          ? _value.selectedOriginalDate
          : selectedOriginalDate // ignore: cast_nullable_to_non_nullable
              as String,
      listOfDates: null == listOfDates
          ? _value._listOfDates
          : listOfDates // ignore: cast_nullable_to_non_nullable
              as List<String>,
      selectedDay: null == selectedDay
          ? _value.selectedDay
          : selectedDay // ignore: cast_nullable_to_non_nullable
              as String,
    ));
  }
}

/// @nodoc

class _$GymStateImpl extends _GymState {
  const _$GymStateImpl(
      {this.isloading = false,
      this.gym = null,
      final List<String>? activities = const [],
      this.selectedActivity = "",
      final List<dynamic>? photos = const [],
      final List<String> originalDates = const [],
      final Map<String, dynamic> scheddules = const {},
      final List<dynamic> availableDates = const [],
      final List<dynamic> listOfSchedules = const [],
      this.defaultDate = "",
      this.selectedOriginalDate = "",
      final List<String> listOfDates = const [],
      this.selectedDay = ""})
      : _activities = activities,
        _photos = photos,
        _originalDates = originalDates,
        _scheddules = scheddules,
        _availableDates = availableDates,
        _listOfSchedules = listOfSchedules,
        _listOfDates = listOfDates,
        super._();

  @override
  @JsonKey()
  final bool isloading;
  @override
  @JsonKey()
  final GymResponse? gym;
  final List<String>? _activities;
  @override
  @JsonKey()
  List<String>? get activities {
    final value = _activities;
    if (value == null) return null;
    if (_activities is EqualUnmodifiableListView) return _activities;
    // ignore: implicit_dynamic_type
    return EqualUnmodifiableListView(value);
  }

  @override
  @JsonKey()
  final String? selectedActivity;
  final List<dynamic>? _photos;
  @override
  @JsonKey()
  List<dynamic>? get photos {
    final value = _photos;
    if (value == null) return null;
    if (_photos is EqualUnmodifiableListView) return _photos;
    // ignore: implicit_dynamic_type
    return EqualUnmodifiableListView(value);
  }

  final List<String> _originalDates;
  @override
  @JsonKey()
  List<String> get originalDates {
    if (_originalDates is EqualUnmodifiableListView) return _originalDates;
    // ignore: implicit_dynamic_type
    return EqualUnmodifiableListView(_originalDates);
  }

  final Map<String, dynamic> _scheddules;
  @override
  @JsonKey()
  Map<String, dynamic> get scheddules {
    if (_scheddules is EqualUnmodifiableMapView) return _scheddules;
    // ignore: implicit_dynamic_type
    return EqualUnmodifiableMapView(_scheddules);
  }

  final List<dynamic> _availableDates;
  @override
  @JsonKey()
  List<dynamic> get availableDates {
    if (_availableDates is EqualUnmodifiableListView) return _availableDates;
    // ignore: implicit_dynamic_type
    return EqualUnmodifiableListView(_availableDates);
  }

  final List<dynamic> _listOfSchedules;
  @override
  @JsonKey()
  List<dynamic> get listOfSchedules {
    if (_listOfSchedules is EqualUnmodifiableListView) return _listOfSchedules;
    // ignore: implicit_dynamic_type
    return EqualUnmodifiableListView(_listOfSchedules);
  }

  @override
  @JsonKey()
  final String defaultDate;
  @override
  @JsonKey()
  final String selectedOriginalDate;
  final List<String> _listOfDates;
  @override
  @JsonKey()
  List<String> get listOfDates {
    if (_listOfDates is EqualUnmodifiableListView) return _listOfDates;
    // ignore: implicit_dynamic_type
    return EqualUnmodifiableListView(_listOfDates);
  }

  @override
  @JsonKey()
  final String selectedDay;

  @override
  String toString() {
    return 'GymState(isloading: $isloading, gym: $gym, activities: $activities, selectedActivity: $selectedActivity, photos: $photos, originalDates: $originalDates, scheddules: $scheddules, availableDates: $availableDates, listOfSchedules: $listOfSchedules, defaultDate: $defaultDate, selectedOriginalDate: $selectedOriginalDate, listOfDates: $listOfDates, selectedDay: $selectedDay)';
  }

  @override
  bool operator ==(dynamic other) {
    return identical(this, other) ||
        (other.runtimeType == runtimeType &&
            other is _$GymStateImpl &&
            (identical(other.isloading, isloading) ||
                other.isloading == isloading) &&
            (identical(other.gym, gym) || other.gym == gym) &&
            const DeepCollectionEquality()
                .equals(other._activities, _activities) &&
            (identical(other.selectedActivity, selectedActivity) ||
                other.selectedActivity == selectedActivity) &&
            const DeepCollectionEquality().equals(other._photos, _photos) &&
            const DeepCollectionEquality()
                .equals(other._originalDates, _originalDates) &&
            const DeepCollectionEquality()
                .equals(other._scheddules, _scheddules) &&
            const DeepCollectionEquality()
                .equals(other._availableDates, _availableDates) &&
            const DeepCollectionEquality()
                .equals(other._listOfSchedules, _listOfSchedules) &&
            (identical(other.defaultDate, defaultDate) ||
                other.defaultDate == defaultDate) &&
            (identical(other.selectedOriginalDate, selectedOriginalDate) ||
                other.selectedOriginalDate == selectedOriginalDate) &&
            const DeepCollectionEquality()
                .equals(other._listOfDates, _listOfDates) &&
            (identical(other.selectedDay, selectedDay) ||
                other.selectedDay == selectedDay));
  }

  @override
  int get hashCode => Object.hash(
      runtimeType,
      isloading,
      gym,
      const DeepCollectionEquality().hash(_activities),
      selectedActivity,
      const DeepCollectionEquality().hash(_photos),
      const DeepCollectionEquality().hash(_originalDates),
      const DeepCollectionEquality().hash(_scheddules),
      const DeepCollectionEquality().hash(_availableDates),
      const DeepCollectionEquality().hash(_listOfSchedules),
      defaultDate,
      selectedOriginalDate,
      const DeepCollectionEquality().hash(_listOfDates),
      selectedDay);

  @JsonKey(ignore: true)
  @override
  @pragma('vm:prefer-inline')
  _$$GymStateImplCopyWith<_$GymStateImpl> get copyWith =>
      __$$GymStateImplCopyWithImpl<_$GymStateImpl>(this, _$identity);
}

abstract class _GymState extends GymState {
  const factory _GymState(
      {final bool isloading,
      final GymResponse? gym,
      final List<String>? activities,
      final String? selectedActivity,
      final List<dynamic>? photos,
      final List<String> originalDates,
      final Map<String, dynamic> scheddules,
      final List<dynamic> availableDates,
      final List<dynamic> listOfSchedules,
      final String defaultDate,
      final String selectedOriginalDate,
      final List<String> listOfDates,
      final String selectedDay}) = _$GymStateImpl;
  const _GymState._() : super._();

  @override
  bool get isloading;
  @override
  GymResponse? get gym;
  @override
  List<String>? get activities;
  @override
  String? get selectedActivity;
  @override
  List<dynamic>? get photos;
  @override
  List<String> get originalDates;
  @override
  Map<String, dynamic> get scheddules;
  @override
  List<dynamic> get availableDates;
  @override
  List<dynamic> get listOfSchedules;
  @override
  String get defaultDate;
  @override
  String get selectedOriginalDate;
  @override
  List<String> get listOfDates;
  @override
  String get selectedDay;
  @override
  @JsonKey(ignore: true)
  _$$GymStateImplCopyWith<_$GymStateImpl> get copyWith =>
      throw _privateConstructorUsedError;
}
